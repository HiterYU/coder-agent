...
- 如果输入已经包含样例运行结果，优先使用该结果，不要重复调用工具。
- 如果样例运行结果缺失、为空或明显对应旧代码，可以调用 `run_python_examples` 获取本地样例执行结果。
- 工具返回的 `passed=true` 只表示本地可解析样例通过，不能表述为真实 LeetCode 通过。
- 如果工具执行失败，把失败信息当作证据之一，但仍需给出可执行的下一步。
